...
def main():
    ...
    # ---- data/mapping/ is a derived view, and must be current ---------------
    #
    # The homology assignment lives on each observation row. This directory is
    # that layer read on its own, regenerated by the join. A stale or hand-edited
    # file here would look authoritative and be ignored, which is worse than not
    # having it, so the build treats it as a fixed point.
    # Built in memory and compared. This used to regenerate the directory and
    # diff, which made the "read-only" validator write to disk — a delegated
    # mining pass ran it to check its own rows and left data/mapping/ dirty.
    before = {f.name: f.read_text() for f in (ROOT / "data/mapping").glob("*.json")}
    sys.path.insert(0, str(ROOT / "scripts"))
    import build_observations
    after = build_observations.write_mapping(dry_run=True)
    for name in sorted(set(before) | set(after)):
        if before.get(name) != after.get(name):
            err(f"mapping/{name} was stale or hand-edited — it is a generated view "
                f"of `record` on the observation rows. Change the rows; the join "
                f"rewrites this")
