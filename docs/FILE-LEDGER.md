...
| `aliases.json` | **derived** | Generated by the join from rows carrying `covers`. One flat file of the names a source uses for something this dataset splits across several records, because the app cannot fetch eighty mapping files to answer "what does the word *deltoid* mean here". Never edit; edit the rows |
