...
## The one open design question — decide before mining at scale

**`data/mapping/` is written and never read.** 78 files, generated by
`--split`, and nothing consumes them: the join resolves each row's homology group
from `record` on the row itself. So the integration layer you designed — homology
decisions living apart from the extraction, changeable without touching it —
**does not exist yet**. The record assignment is baked onto every observation row.
...
| | What it means | Cost |
|---|---|---|
| **Wire mapping in** | `mapping/` becomes authoritative for named rows; `record` stays only on the 146 unnamed ones. Re-homologising is one line in one file | Adding a muscle touches two files, not one |
| **Delete `mapping/`** | `record` on the row is the assignment, and re-homologising is a scripted find-and-replace across the observation files | Honest and simple, but it is not the layered model you described |
| **Derive both** | Keep `record` on rows as now, and generate `mapping/` as a queryable *view* of the homology decisions — read-only, never edited | No new friction; the mapping stops being a lie without becoming a second source of truth |

The third is the smallest change and the least likely to be wrong. It also makes
the eventual UI build step trivially able to read the homology layer on its own,
which is what the mapping was for.
